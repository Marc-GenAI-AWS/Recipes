...
#
# `generations/` and the `async/` prefixes are created on first write and do
# not need to pre-exist. Run `preflight.py` after populating to verify layout
# before deploying.
...
#   - s3:PutObject on s3://{BUCKET}/{OUTPUT_PREFIX}/* and s3://{BUCKET}/async/*
...
# Async inference input/output locations.
ASYNC_INPUT_PREFIX = f"s3://{BUCKET}/async/input/"
ASYNC_OUTPUT_PATH = f"s3://{BUCKET}/async/output/"
ASYNC_ERROR_PATH = f"s3://{BUCKET}/async/errors/"

# SageMaker naming (immutable once created -- bump the version suffix to redeploy).
# Distinct from the real-time project's names so both endpoints can coexist.
MODEL_NAME = "wan22-ti2v-5b-v3"
ENDPOINT_CONFIG_NAME = "wan22-ti2v-5b-cfg-v3"
ENDPOINT_NAME = "wan22-ti2v-5b-async"
