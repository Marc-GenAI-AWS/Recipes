...
Captured from a real customer validation of **data in one AZ, GPU training in another AZ, via SageMaker Training Jobs**. Applicable to any customer with similar constraints (e.g. P5 capacity only available in a specific AZ while storage lives elsewhere).
...
# Writes the resulting AMI ID to SSM Parameter Store at /cross-az-fsx/bionemo-prep-ami-id
